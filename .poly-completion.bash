...
_poly_complete() {
  ...
  if [[ ${COMP_WORDS[1]} == "import" ]]; then
    ...
    COMPREPLY=( $( compgen -W 'dump-remote-db import-remote-databases-to-local import-local-databases-to-remote deploy update copy-remote-uploads copy-local-uploads npm-install open-sites symlink-uploads exec-command' -- $cur ) );
