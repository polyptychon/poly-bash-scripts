...
_poly_complete() {
  ...
  if [[ ${COMP_WORDS[1]} == "import" ]]; then
    ...
    COMPREPLY=( $( compgen -W 'import-remote-databases-to-local import-local-databases-to-remote deploy update copy-remote-uploads copy-local-uploads open-local-sites symlink-uploads' -- $cur ) );
